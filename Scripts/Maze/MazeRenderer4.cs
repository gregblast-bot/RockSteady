...
public class MazeRenderer4 : MonoBehaviour
{ 
    [SerializeField]
    [Range(1, 50)]
    ...
    [SerializeField]
    private Transform bouncyWallPrefab = null; // Bouncy wall object
    ...
    public string playerTag; // SYlvester Stone's tag
    ...
    private TimerWithCoroutine elapsedTime; // Use this to trigger end of game if timer elapsed
    ...
    public int amount2Pool;
    private int count2Light = 0;
    private int count2Strike = 0;  
    private int death = 0;
    ...
    void Start()
    {
        ...
        {
            tmp1 = Instantiate(object2Pool);
            tmp1.SetActive(false);
            pooledObjects.Add(tmp1);
        }
        tmp2 = Instantiate(poolOfBlood);
        ...
        elapsedTime = this.GetComponent<TimerWithCoroutine>();
        ...
        // Subscribe to win/time event for smooth scene transition
        onWin.triggerWinEvent.AddListener(() => subscribe2Change());
        elapsedTime.OnTimeOutEvent.AddListener(() => subscribe2Time());

        playerTag = sylvester.gameObject.tag;
    }
    ...
    void FixedUpdate()
    {
        ...
        // Ray setup and screen position of sylvester
        if (sylvester != null && death < 3)
        {
            var pos = sylvester.transform.localPosition;
            Vector3 screenPos = Camera.main.WorldToScreenPoint(sylvester.transform.position);
            screenPos.x = screenPos.x - 12.5f;
            screenPos.y = screenPos.y - 12.5f;
            Ray ray;
            ray = Camera.main.ScreenPointToRay(screenPos);
            RaycastHit hit;
            GameObject lightning = getPooledObject();
            GameObject blood = getBloodPooledObject();

            // Deactivate pooled objects every 15 seconds
            count2Light += 1;
            if (count2Light >= 750)
            {
            ...
            // Activate sound clip every 10 seconds
            count2Strike += 1;
            if (count2Strike == 350){
                TriggerSound.PlayOneShot(TriggerClip, volume);
            }

            // Activate pooled objects every 10 seconds
            if (count2Strike >= 500)
            {
                // Object pool this lightning
                if (Physics.Raycast(ray, out hit))
                {
                    if (lightning != null)
                    {
                        Vector3 _p = new Vector3(hit.point.x, 0f, hit.point.z);
                        lightning.transform.position = _p;
                        lightning.SetActive(true);
                        count2Strike = 0;
                    }
                    if (hit.collider.tag == playerTag && blood != null)
                    {
                        // Logic for kill
    ...
    {
        // Variables to hold whether we should place a regular or bouncy wall
        int countL = 0, countT = 0, countR = 0, countB = 0;
            ...
            for (int j = 0; j < height; ++j)
            {
                ...
                if (cell.HasFlag(WallState.LeftWall))
                {
                    ...
                    if (i != width - 3 || j != randExitPosY)
                    {
                        ...
                        if (countL%2 == 1)
                        {
                            leftWall = Instantiate(bouncyWallPrefab, transform) as Transform;
                        }
                ...
                if (cell.HasFlag(WallState.TopWall))
                {
                    ...
                    if (countT % 2 == 1)
                    {
                        topWall = Instantiate(bouncyWallPrefab, transform) as Transform;
                    }
                ...
                if (i == width - 1)
                {
                    if (cell.HasFlag(WallState.RightWall))
                    {
                        ...
                        if (countR % 2 == 1)
                        {
                            rightWall = Instantiate(bouncyWallPrefab, transform) as Transform;
                        }
                ...
                if (j == 0)
                {
                    if (cell.HasFlag(WallState.BottomWall))
                    {
                        ...
                        if (countB % 2 == 1)
                        {
                            bottomWall = Instantiate(bouncyWallPrefab, transform) as Transform;
                        }
    ...
    private GameObject getBloodPooledObject()
    {
        if (!pooledObjects[3].activeInHierarchy)
        {
            return pooledObjects[3];
        }
        return null;
    }
